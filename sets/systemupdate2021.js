...
cardSet[31004] = {
  ...
  AIRunEventExtraPotential: function(server,potential) {
	  ...
	  if (server == corp.archives) { 
		  ...
		  if (runner.AI._rootKnownToContainCopyOfCard(server, "危海网阵")) return 0;
...
cardSet[31005] = {
  ...
  AIPreferredInstallChoice: function (
	...
	if (runner.AI._copyOfCardExistsIn("血块", InstalledCards(runner))) return -1; //don't install
    return 0; //do install
  },
};
...
cardSet[31007] = {
  ...
  AIPreferredInstallChoice: function (
	...
	var existingCopy = runner.AI._copyOfCardExistsIn("小恶魔", InstalledCards(runner));
	if (existingCopy) {
  ...
  Resolve: function (params) {
	...
	if (CheckCredits(runner,1)) {
		...
		if (runner.AI != null) {
			...
			runner.AI.preferred = { title: "社交", option: choice }; //title must match currentPhase.title for AI to fire
		}
	}
  },
  ...
  responseOnEncounter: {
    Enumerate: function (card) {
        ...
        var choices = [
          ...
          { id: 1, label: "继续", button: "继续", alt:"continue" },
        ];
		return choices;
	},
    Resolve: function (params) {
...
cardSet[31024] = {
  ...
  responseOnRunnerTurnBegins: {
    Enumerate: function() {
	  ...
	  choices.push({button:"继续", label:"继续"});
...
cardSet[31031] = {
  ...
  AISharedPreferredWord: function() {
	...
	if (runner.identityCard.title == '芮尔·姬特·佩德拉：超凡入圣' && !runner.identityCard.usedThisTurn) {
	...
	for (var j=2; j>-1; j--) {
      for (var i = 0; i < installedRunnerCards.length; i++) {
        ...
        if (CheckSubType(installedRunnerCards[i], stToCheck)) {
		  options.splice(j, 1);
		  break;
		} else if (installedRunnerCards[i].title == "变色龙" && installedRunnerCards[i].chosenWord == options[j]) {
		  options.splice(j, 1);
		  break;
		}
	  }
...
cardSet[31035] = {
    ...
    Resolve: function () {	
	  ...
	  if (choices.length > 0) {
	    ...
	    if (runner.AI != null) {
		  ...
		  runner.AI.preferred = { title: "伊索的典当铺", option: choice };
	    }
	  }
    },
    text: "伊索的典当铺",
  ...
  AISharedWouldTrash: function(card) {
	...
	if (card.title == "食谱" || card.title == "Avgustina Ivanovskaya") {
	...
	else if (card.title == "码头区通行证") {
		if (corp.HQ.ice.length > 3) {
			if (!runner.AI._copyOfCardExistsIn("测试暗门", runner.rig.programs)) return true;
		}
	}
	//Red Team: less than 4 credits left
	else if (card.title == "假想敌") {
	...
	else if (card.title == "远程工作合约") {
		if (card.credits < 4) return true;
	}
	//Security Testing: less open servers than these installed
	else if (card.title == "安全测试") {
		var numInstalled = 0;
		for (var j = 0; j < runner.rig.resources.length; j++) {
		  if (runner.rig.resources[j].title == "安全测试") {
			numInstalled++;
		  }
		}
...
cardSet[31037] = {
  ...
  AIRunEventExtraPotential: function(server,potential) {
	//require successful run
	if (runner.AI._rootKnownToContainCopyOfCard(server, "危海网阵")) return 0; // don't use
...
cardSet[31045] = {
		  ...
		  if (card.rezzed) {
			  if (CheckSubType(card, "Bioroid")) {
				  if (CheckCardType(card, ["ice"])) {
					  ...
					  if (card.title != "罗波那 1.0") return true;
				  }
			  }
		  }
		  return false;
  ...
  AIImplementIce: function(rc, result, maxCorpCred, incomplete) {
	...
	for (var i=0; i<installedCards.length; i++) {
		...
		if (otherCard.rezzed) {
			if (otherCard.title == "伊莱 1.0") {
				worstSR = ["endTheRun"];
			}
			else if (otherCard.title == "安塞尔 1.0" || otherCard.title == "布兰 1.0") {
					...
					if (currentPhase.identifier == "Run 4.5" && approachIce < 1) {
						...
						if (corp.AI._runnerMayWinIfServerBreached(attackedServer)) {
							...
							if (attackedServer == corp.RnD) {
								if (corp.AI._copyOfCardExistsIn("造物者之眼", runner.resolvingCards)) return [{}];
								return [];
							}
  ...
  responseOnAccess: {
    ...
    Resolve: function () {
	  ...
	  var SnareImplementation = function() {
		...
		choices.push({ id:1, button:"继续", label:"继续"});
		DecisionPhase(
		  corp,
		  choices,
		  decisionCallback,
	  ...
	  var rdp = DecisionPhase(runner,[{button:"继续",label:"继续"}],function(){
  ...
  responseOnScored: {
    Resolve: function () {
      if (intended.score == this) {
		...
		var continueChoice = { id: 2, label: "继续", button: "继续" };
		...
		if (corp.AI) {
			...
			corp.AI.preferred = { title: "获取许可", option: choice }; //title must match currentPhase.title for AI to fire
		}
      }
    },
  },
...
cardSet[31062] = {
  ...
  AIAdvancementLimit: function() {
	...
	if (thisServer) {
		var existingSanSan = corp.AI._copyOfCardExistsIn("圣加州城际网阵", thisServer.root);
		if (existingSanSan) {
...
cardSet[31077] = {
  ...
  AIWorthwhileIce: function(server, purpose) {
	if (purpose == "install") {
	  ...
	  if (server) {
		if (corp.AI._copyOfCardExistsIn("冰墙", server.ice)) return false; //not worthwhile
	  }
	}
...
cardSet[31079] = {
  ...
  AIDefensiveValue: function(server) {
	...
	if (server == corp.HQ) {
		...
		else if (corp.AI._copyOfCardExistsIn("安全测试",runner.rig.resources)) return 1.5;
  ...
  responseOnCorpTurnBegins: {
    ...
    Resolve: function (params) {
      ...
      if (corp.AI != null) {
        ...
        corp.AI.preferred = { title: "潜意识信号", option: choice }; //title must match currentPhase.title for AI to fire
      }		
    },
    text: "潜意识信号",
	availableWhenInactive:true,
...
cardSet[26094] = {
  title: "每日播报",
  imageFile: "26094.png",
  cardText: "When you install this resource, load 8[credit] onto it. When it is empty, trash it. When your turn begins, take 2[credit] from this resource.",
  elo: 1849,
  player: runner,
  faction: "Neutral",
  influence: 0,
...
cardSet[26095] = {
  title: "努卡·弗罗丽克博士",
  imageFile: "26095.png",
  cardText: "When you install this resource, load 2 power counters onto it. When it is empty, trash it.\n[click], hosted power counter: Draw 3 cards.",
  elo: 1849,
  player: runner,
  faction: "Shaper",
  influence: 2,
  ...
  abilities: [
    {
      ...
      Enumerate: function () {
        if (!CheckActionClicks(runner, 1)) return [];
        if (!CheckCounters(this, "power", 1)) return []; //because it doesn't say 'take *up to* ...'
        return [{}];
      },
      Resolve: function (params) {
        SpendClicks(runner, 1);
...
cardSet[26097] = {
  ...
  responseOnAccess: {
    ...
    Resolve: function () {
	    this.storedFaceUp = this.faceUp;
      Purge();
      if(CheckInstalled(this) && this.rezzed && CheckActive(this)) {
        Damage("net", 1, true);
      }
    },
  },
