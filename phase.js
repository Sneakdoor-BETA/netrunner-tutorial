...
phaseTemplates.mulligan = {
  ...
  Resolve: {
    m: function () {
      SetHistoryThumbnail("", "调度");
      Log(PlayerName(activePlayer) + " took a mulligan");
      Mulligan();
    },
    n: function () {
      SetHistoryThumbnail("", "保留");
...
phases.corpEndDraw = {
  ...
  identifier: "Corp 1.3",
  Init: function () {
    SetHistoryThumbnail("Corp_back.png", "抽牌");
  }, //auto-draw
  Resolve: {
    n: function () {
...
phases.corpActionMain = {
  ...
  Resolve: {
    ...
    gain: function () {
      SetHistoryThumbnail("credit.png", "获得");
      SpendClicks(corp, 1);
      GainCredits(corp, 1);
      IncrementPhase();
    },
    install: function (params) {
      Install(
        ...
        function () {
          ...
          else SetHistoryThumbnail("Corp_back.png", "安装");
          SpendClicks(corp, 1);
        },
        this
      );
    ...
    play: function (params) {
      Play(
        ...
        function (finishResolve) {
          SetHistoryThumbnail(params.card.imageFile, "打出");
    ...
    advance: function (params) {
      SetHistoryThumbnail("Corp_back.png", "推进");
      SpendClicks(corp, 1);
      SpendCredits(
        corp,
        1,
    ...
    trash: function (params) {
      SetHistoryThumbnail(params.card.imageFile, "销毁");
      SpendClicks(corp, 1);
      SpendCredits(
        corp,
        2,
    ...
    trigger: function (params) {
      TriggerAbility(
        ...
        params.ability,
        function () {
          SetHistoryThumbnail(params.card.imageFile, "使用");
    ...
    purge: function () {
      SetHistoryThumbnail("generic_red.png", "清毒");
      SpendClicks(corp, 3);
      IncrementPhase();
      Purge();
    },
  ...
  text: {
    draw: "[时点]: 抽一张卡牌",
    gain: "[时点]: 获得1信用点",
    install: "[时点]: 安装一个议案、资产、升级或防火墙",
    play: "[时点]: 打出一个事务",
    advance: "[时点], 1[信用点]: 推进一张卡牌",
    trash: "[时点], 2[信用点]: 销毁一个资源",
    trigger: "触发一个[时点]能力",
    purge: "[时点],[时点],[时点]: 清除病毒指示物",
  },
};
...
phases.runnerActionMain = {
  ...
  Resolve: {
    draw: function () {
      SetHistoryThumbnail("Runner_back.png", "抽牌");
    ...
    install: function (params) {
      Install(
        ...
        false,
        function () {
          SetHistoryThumbnail(params.card.imageFile, "安装");
          SpendClicks(runner, 1);
        },
        this
      );
    ...
    remove: function () {
      SetHistoryThumbnail("tag.png", "移除");
      SpendClicks(runner, 1);
      SpendCredits(
        runner,
        2,
  ...
  text: {
    draw: "[时点]: 抽一张卡牌",
    gain: "[时点]: 获得1信用点",
    install: "[时点]: 安装一个程序、硬件或资源",
    play: "[时点]: 打出一个事件",
    advance: "[时点], 2[信用点]: 移除1枚锁定标记",
    run: "[时点]: 进行一次潜袭",
    trigger: "触发一个[时点]能力",
  },
};
...
phases.runBreachServer = {
  player: runner,
  title: "潜袭：侵入", //was 'Access' (i.e. access cards in server) but Nisei changed it so it's not confused with each individual access
  identifier: "Run 5.2",
  Init: function() {
...
phases.runAccessingCard = {
  ...
  Init: function () {
    ...
    if (viewingPlayer == runner) {
      ...
      SetHistoryThumbnail(accessingCard.imageFile, "读取");
    } else SetHistoryThumbnail("Corp_back.png", "读取");
  ...
  Resolve: {
    ...
    steal: function () {
      SetHistoryThumbnail(accessingCard.imageFile, "窃取");
      Steal();
    },
    trigger: function (params) {
      TriggerAbility(params.card, params.ability);
