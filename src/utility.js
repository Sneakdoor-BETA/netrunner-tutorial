...
function PlayerWin(player, msgstr) {
  ...
  var winnerMessage = "";
  if (winner == corp) winnerMessage = "公司获胜";
  else winnerMessage = "潜袭者获胜";
  ...
  winPhase.Enumerate["再次游戏"] = function () {
	if (debugging && runner.AI && corp.AI) alert("Game completed");
    return [{}];
  };
  winPhase.Resolve["再次游戏"] = function () {
    location.reload(); //restart game
  };
  ChangePhase(winPhase);
  Render();
  SetHistoryThumbnail("", "游戏结束");
...
function DeckBuild(
  ...
  if (identityCard.player == runner) {
	  ...
	  var influenceUsed = 0;
	  var infMult = 1.0;
	  if (identityCard.title == '芮尔·姬特·佩德拉：超凡入圣') infMult = 0.5; //account for lower max inf
	  //consoles
	  var consoleCards = [];
	  ...
	  if (identityCard.title == '芮尔·姬特·佩德拉：超凡入圣') numDecoders = RandomRange(2, 3); //special case
	  cardsAdded = cardsAdded.concat(DeckBuildRandomly(
		identityCard,
		decoderCards,
		destination,
