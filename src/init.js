...
function Init() {
  ...
  corp.HQ = NewServer("总部", true); //corp.HQ.cards is corp hand
  corp.RnD = NewServer("研发中心", true);
  corp.archives = NewServer("档案库", true);
  corp.remoteServers = [];
...
function NicelyFormatCommand(cmdstr) {
  ...
  if (cmdstr == "Trash" || cmdstr == "trash") {
    cmdstr = "销毁";
  } else if (cmdstr == "Continue" || cmdstr == "continue") {
    cmdstr = "继续";
  } else if (cmdstr == "Run" || cmdstr == "run") {
    cmdstr = "发起潜袭";
  } else if (cmdstr == "Purge" || cmdstr == "purge") {
    cmdstr = "清除病毒";
  } else if (cmdstr == "n") {
	if (currentPhase.identifier == "Run Accessing") {
      if (ChoicesAccess().length > 0) cmdstr = "下一个";
      else cmdstr = "结束";
	}
      ...
      if (viewingPlayer == corp) {
        if (approachIce > 0) cmdstr = "继续到接驳防火墙阶段";
        else cmdstr = "继续到成功阶段";
      } else cmdstr = "侵入";
    } else if (
      viewingPlayer == corp &&
      ...
      } else cmdstr = "继续到下一个行动";
    } else if (viewingPlayer == corp && currentPhase.identifier == "Corp 3.2")
      cmdstr = "回合结束";
    else if (typeof phaseOptions.m !== "undefined") cmdstr = "保留";
    else cmdstr = "继续";
  } else if (cmdstr == "m") cmdstr = "调度";
  else if (cmdstr == "jack") cmdstr = "退出";
...
function ExecuteChosen(chosenCommand) {
  if (chosenCommand != null) {
    ...
    //have decided I prefer to not show the resolving text as it might just confuse the user (it usually said "继续")
    ...
    if (viewingPlayer == activePlayer) {
      if (activePlayer == runner) {
        if (oldFooterText == "读取") footerText = "读取卡牌";
        else if (oldFooterText == "弃除") footerText = "拖至堆阵";
		else if (oldFooterText == "拖至存储栈") footerText = oldFooterText;
      } //corp
      else {
        if (oldFooterText == "弃除") footerText = "拖至档案库";
        else if (oldFooterText == "销毁") footerText = "选择销毁的卡牌";
        else if (oldFooterText == "拖至研发中心") footerText = oldFooterText;
      }
	  if (oldFooterText == "选择一层防火墙") footerText = oldFooterText;
	  if (oldFooterText == "选择移动来源卡牌") footerText = oldFooterText;
	  if (oldFooterText == "选择要展示的卡牌") footerText = oldFooterText;
	  if (oldFooterText == "选择要放置的卡牌") footerText = oldFooterText;
	  if (oldFooterText == "拖动卡牌以安装") footerText = oldFooterText;
    ...
    if (AgendaPoints(corp) >= AgendaPointsToWin())
      PlayerWin(
        corp,
        "公司已获得 " + AgendaPointsToWin() + " 点议案分数"
      );
    else if (AgendaPoints(runner) >= AgendaPointsToWin())
      PlayerWin(
        runner,
        "潜袭者已获得 " + AgendaPointsToWin() + " 点议案分数"
      );
