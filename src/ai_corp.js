...
class CorpAI {
  ...
  _iceIsDisabled(iceCard) {
	...
	var femmes = this._copiesOfCardIn("蛇蝎美人", runner.rig.programs);
  ...
  _iceInstallScore(ice, serverToInstallTo) {
	...
	ret += RezCost(ice);
	//special specifics
	if (ice.title == "围栏" && serverToInstallTo != null && typeof(serverToInstallTo.cards) !== 'undefined') ret -= 3; //arbitrary, encourage to save Palisade for remotes
  ...
  _archivesIsBackdoorToHQ() {
	  if (this._copyOfCardExistsIn("测试暗门",runner.rig.programs)) {
		  return true;
	  }
	  return false;
  }
  ...
  _extraThreatOnRnD() {
	  var conduit = this._copyOfCardExistsIn("灵媒",runner.rig.programs);
	  if (conduit) {
  ...
  _advancementLimit(card,server) {
	...
	if (server && typeof card.advancementRequirement !== "undefined" && ret == card.advancementRequirement) {
		var sanSan = this._copyOfCardExistsIn("圣加州城际网阵", server.root);
		if (sanSan) {
  ...
  _bestRecurToHQOption(optionList,serverUnderThreat,useNowIfPossible=false) {
	  ...
	  if (serverUnderThreat == corp.HQ && this._agendasInHand() > 0) {
		  ...
		  if (CheckCredits(corp,4)) {
			  for (var i=0; i<optionList.length; i++) {
				if (optionList[i].card) {
				  if (optionList[i].card.title == "圈套!") return optionList[i];
				}
			  }
		  }
  ...
  _potentialDamageOnBreach(server) {
	...
	if (this._agendaPointsInServer(server) > 0 && corp.identityCard.title=="人间会社：自我进化") {
		if (!this._runnerMayWinIfServerBreached(server)) ret++;
	}
	//2+advancement if it is an active Urtica
	var urtica = this._copyOfCardExistsIn("荨麻暗码", server.root);
	...
	for (var i=0; i<corp.scoreArea.length; i++) {
		if (corp.scoreArea[i].title == "利刃屋" && !corp.scoreArea[i].usedThisRun) ret++;
	}
	//3 for Snare! if affordable (for now, assumes exactly one will be accessed)
	var snare = null;
	if (this._copyOfCardExistsIn("圈套!", server.root)) snare = true;
	else if (typeof server.cards != 'undefined') {
		if (this._copyOfCardExistsIn("圈套!", server.cards)) snare = true;
	}
	...
	if (this._copyOfCardExistsIn("北斋网阵", server.root) && creditsLeft > 1) {
		creditsLeft -= 2;
		ret++;
	}
  ...
  _cardProtectionValue(
    ...
    if (CheckCardType(card, ["ice"])) {
		...
		if ( card.rezzed || Credits(corp) >= RezCost(card)) {
			...
			if (card.title == "冰墙") ret += 0.5 * card.advancement;
    ...
    for (var i = 0; i < corp.remoteServers.length; i++) {
      ...
      for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
        if (CheckCardType(corp.remoteServers[i].root[j], ["agenda", "asset"])) {
		  ...
		  else if (CheckCounters(corp.remoteServers[i].root[j], "advancement", 2) && this._copyOfCardExistsIn("光影诡计",corp.HQ.cards.concat(corp.RnD.cards))) hasRoom = false;
        }
      }
  ...
  _economyCards(affordableOnly=false) {
	var economyCards = [];
	if (!affordableOnly || corp.creditPool >= 3) economyCards.push("名流的馈赠"); //we have this first because playing other cards first would reduce cards in hand
    ...
    if (!affordableOnly || corp.creditPool >= 10) economyCards.push("政府补贴");
    if (!affordableOnly || corp.creditPool >= 5) economyCards.push("对冲基金");
    if ( (!affordableOnly || corp.creditPool >= 5) && (this._agendasInHand() < corp.HQ.cards.length - 1) )
      economyCards.push("自我反省"); //only if there is at least 1 non-agenda card (other than this) in HQ
    if (!affordableOnly || corp.creditPool >= 2) economyCards.push("玛丽莲营销计划");
    if (!affordableOnly || corp.creditPool >= 3) economyCards.push("表岩屑开采许可");
    if (!affordableOnly || corp.creditPool >= 2) economyCards.push("尼可营销计划");
    if (!affordableOnly || corp.creditPool >= 2) economyCards.push("PAD Campaign");
    if (corp.creditPool < corp.HQ.cards.length) economyCards.push("消费预测智慧商城"); //simple check whether to use for econ or save for draw
	return economyCards;
  }
  ...
  _bestMainPhaseEconomyOption(optionList) {
	...
	if (optionList.indexOf("advance") > -1) {
		var oaktown = this._copyOfCardExistsIn("革新奥克敦", installedCards);
    ...
    if (MaxHandSize(corp) - PlayerHand(corp).length > 0) {
	  ...
	  drawCards.push("舆论操盘手");
      drawCards.push("疾跑");
	  drawCards.push("每日商业秀");
	  if (corp.HQ.cards.length < corp.creditPool) drawCards.push("消费预测智慧商城"); //simple check whether to use for draw or save for econ
  ...
  _sufficientEconomy(
    ...
    var rootUseCosts = [
      ...
      { title: "圈套!", cost: 4 },
      { title: "荨麻暗码", cost: 0 },
      { title: "逐月狼秘密开发部", cost: 2 },
      { title: "无明虚空", cost: 0 },
      { title: "清算所", cost: 0 },
	  { title: "浪人", cost: 0 },
      { title: "北斋网阵", cost: 2 },
	  { title: "撤销账户", cost: 0 },
	  { title: "圣加州城际网阵", cost: 6 },
  ...
  _rankedInstallOptions(
	...
	//or even maybe...these?
	var snare = this._copyOfCardExistsIn("圈套!",cards);
  ...
  _iceWorthRezzing(card, currentRezCost, server) {
	  ...
	  if (server) {
		...
		var costyAmbushes = [{title:"圈套!",cost:4}];
	  ...
	  if (runner.resolvingCards.length > 0) {
		  var copyOfInsideJob = this._copyOfCardExistsIn("内线操作", runner.resolvingCards);
		  if (copyOfInsideJob) {
  ...
  Phase_EOT(
    ...
    if (optionList.indexOf("rez") > -1) {
      ...
      var cardsToRezIfDuplicate = ["舆论操盘手"];
  ...
  Phase_PostAction(optionList) {
    if (optionList.indexOf("rez") > -1) {
      ...
      var cardsToRezPostAction = ["圣加州城际网阵"];
      if (this._clicksLeft() > 0)
        cardsToRezPostAction.push("表岩屑开采许可", "浪人", "撤销账户");
  ...
  _potentialOperationDamageDirections(point) {
	  ...
	  //Neurospike
	  cardinhand = this._copyOfCardExistsIn("神经突刺",point.handCards);
	  ...
	  cardinhand = this._copyOfCardExistsIn("强硬反击",point.handCards);
	  ...
	  if (point.corpCredits > 3 && point.corpClicks > 0) {
		  cardinhand = this._copyOfCardExistsIn("生化人劳工",point.handCards);
		  indexinhand = point.handCards.indexOf(cardinhand);
		  if (indexinhand > -1) {
			newhandcards = point.handCards.concat([]); //make a copy of the array
			newarchivescards = newarchivescards.concat(newhandcards.splice(indexinhand,1)); //move played card
	  ...
	  if (point.corpClicks > 1) {
		  cardinhand = this._copyOfCardExistsIn("归档的记忆",point.handCards);
		  indexinhand = point.handCards.indexOf(cardinhand);
		  if (indexinhand > -1) {
			var cardsToRecur = ["神经突刺","强硬反击","生化人劳工"];
  ...
  _potentialOperationDamageThisTurn(output=[]) {
	  ...
	  for (var i=0; i<corp.resolvingCards.length; i++) {
		  if (corp.resolvingCards[i].title == "神经突刺") startingDamage += corp.resolvingCards[i].printedAgendaPointsThisTurn;
		  else if (corp.resolvingCards[i].title == "强硬反击") startingDamage += corp.resolvingCards[i].printedAgendaPointsLastTurn;
	  }
	  var startingPoint = {
  ...
  _potentialAdvancementDirections(point,options=null) {
	  ...
	  if (point.corpCredits > 0 && point.corpClicks > 0) {
		  ...
		  if (point.advancementSoFar < 1 && corp.identityCard.title == "威兰财团：建造不朽") newCredits += 2
	  ...
	  if (card && point.corpCredits > 0 && point.corpClicks > 0) {
		  var slih = this._copyOfCardExistsIn("无瑕发布会",point.handCards);
		  if (slih) {
	  ...
	  //Psychographics
	  //already resolving
	  var pgres = this._copyOfCardExistsIn("心理统计分析",corp.resolvingCards);
	  ...
	  //still in hand
	  var pgih = this._copyOfCardExistsIn("心理统计分析",point.handCards);
	  ...
	  //Trick of Light
	  if (point.corpCredits > 0 && point.corpClicks > 0) {
		  var tlih = this._copyOfCardExistsIn("光影诡计",point.handCards);
	  ...
	  //Biotic Labor
	  if (point.corpCredits > 3 && point.corpClicks > 0) {
		  var blih = this._copyOfCardExistsIn("生化人劳工",point.handCards);
  ...
  //or null if none
  _useWhenTaggedCard() {
	  var useWhenTaggedCards = ["打击报复", "消费预测智慧商城"];
  ...
  Phase_Main(optionList) {
	...
	if (!almostDoneAgenda && !almostDoneHostileAsset) {
		...
		if (optionList.indexOf("purge") > -1) {
		  ...
		  var virus_min = 2;
		  var installedRunnerCards = InstalledCards(runner);
		  if (this._copyOfCardExistsIn("血块",installedRunnerCards)) {
					  ...
					  if (!cardToPlay) {
						...
						if (spareCredAfterExpensiveAdv >= 0) {
							...
							if (!worthExpensiveFastAdvance) {
								...
								var copyOfPublicTrail = this._copyOfCardExistsIn("公开足迹",arrayToCheckForCombo);
								if (spareCredAfterExpensiveAdv >= 4 && card.title == "轨道压制" && copyOfPublicTrail && copyOfPublicTrail.successfulRunLastTurn) worthExpensiveFastAdvance=true;
								//and after score:
								//when an agenda is scored as Precision Design, take into account recur 1 card from archives
								if (corp.identityCard.title == "哈斯生化：精密设计") arrayToCheckForCombo = arrayToCheckForCombo.concat(corp.archives.cards);
								//when the agenda scored is Offworld Office, extra money exists
								if (card.title == "地外办公室" || card.title == "恶意收购") spareCredAfterExpensiveAdv += 7;
								//now check
								if (spareCredAfterExpensiveAdv >= 3 && this._copyOfCardExistsIn("神经突刺",arrayToCheckForCombo)) worthExpensiveFastAdvance=true;
								if (card.title == "明日头条" && this._potentialTagPunishment(runner.tags+1,1,spareCredAfterExpensiveAdv)) worthExpensiveFastAdvance=true;
							}
							if (worthExpensiveFastAdvance) {
							  cardToPlay = this._copyOfCardExistsIn(
								"生化人劳工",
  ...
  Choice(optionList, choiceType) {
    ...
    if (ret < 0) {
      ...
      else if (currentPhase.title == "安装前销毁")
