  ...
  Resolve: function (params) {
    ...
    if (corp.AI != null) {
      ...
      if (
        ...
        choice = choices[0];
      corp.AI.preferred = { title: "野猫罢工", option: choice };
    }
  },
...
cardSet[30003] = {
  ...
  abilities: [
    {
      ...
      Enumerate: function () {
		...
		if (runner.AI && runner.AI._rootKnownToContainCopyOfCard(GetServer(accessingCard),"荨麻暗码")) return []; //special case: don't use this if there's an urtica in server
  ...
  AIReducesTrashCost: function(card) {
	...
	if (runner.AI._rootKnownToContainCopyOfCard(GetServer(card),"荨麻暗码")) return 0; //special case: don't use this if there's an urtica in server
...
cardSet[30008] = {
  ...
  AIPermitMoreLeeches: function(installedRunnerCards) {
	...
	for (var i=0; i<installedRunnerCards.length; i++) {
		if (GetTitle(installedRunnerCards[i]) == "水蛭") numLeeches++;
	}
    ...
    Reveal(
      ...
      function () {
        ...
        if (runner.AI != null && choices.length > 0) {
          ...
          runner.AI.preferred = { title: "互惠互利", option: choice }; //title must match currentPhase.title for AI to fire
        }
      },
      this
    );
...
cardSet[30024] = {
  ...
  responseOnRunEnds: {
    ...
    Resolve: function () {
        ...
        if (runner.AI != null) {
          ...
          runner.AI.preferred = { title: "灵媒", option: choice }; //title must match currentPhase.title for AI to fire
        }
    },
  },
  //[click]: Run R&D.
  ...
  AIWastefulToInstall: function() {
	  for (var j = 0; j < runner.rig.programs.length; j++) {
		if (runner.rig.programs[j].title == "灵媒") {
...
cardSet[30032] = {
  ...
  AIWastefulToInstall: function() {
	  for (var j = 0; j < runner.rig.programs.length; j++) {
		if (runner.rig.programs[j].title == "蜉蝣") {
  ...
  AIWouldTrigger: function () {
    ...
    for (var i = 0; i < runner.rig.resources.length; i++) {
      if (runner.rig.resources[i] !== this) {
        if (runner.rig.resources[i].title == "智能装置经销商") {
...
cardSet[30035] = {
  ...
  responseOnScored: {
    Enumerate: function () {
      ...
      ret.push({ card: null, label: "继续", button: "继续" });
      return ret;
    },
    Resolve: function (params) {
      if (params.card !== null) {
  ...
  subroutines: [
    ...
    },
    {
      ...
      Resolve: function () {
        ...
        choicesA.push({ id: 2, label: "继续", button: "继续" });
        ...
        if (corp.AI != null) {
          ...
          corp.AI.preferred = { title: "安塞尔 1.0", option: choice }; //title must match currentPhase.title for AI to fire
        }
      },
      visual: { y: 113, h: 31 },
    },
      ...
      Resolve: function (params) {
        ...
        if (corp.AI != null) {
          ...
          if (archivesOptions.length > 0 && handOptions.length > 0)
            choice = choicesA[1];
          corp.AI.preferred = { title: "布兰 1.0", option: choice }; //title must match currentPhase.title for AI to fire
        }
      },
      visual: { y: 105, h: 46 },
    },
    ...
    Draw(corp, 3, function() {
		...
		var decisionCallbackA = function (params) {
		  ...
		  if (corp.AI != null) {
			...
			corp.AI.preferred = { title: "疾跑", option: choiceB }; //title must match currentPhase.title for AI to fire
		  }
		};
		...
		if (corp.AI != null) {
		  ...
		  corp.AI.preferred = { title: "疾跑", option: choiceA }; //title must match currentPhase.title for AI to fire
		}
	},this);
  },
};
  ...
  responseOnApproachServer: {
    ...
    Resolve: function (params) {
      ...
      if (runner.AI != null) {
        ...
        runner.AI.preferred = { title: "逐月狼秘密开发部", option: choice }; //title must match currentPhase.title for AI to fire
      }
    },
...
cardSet[30044] = {
  ...
  SharedDecisionCallback: function(params) {
	...
	choices.push({ label: "继续", button: "继续" });
	for (var i = 0; i < choices.length; i++) {
	...
	if (corp.AI != null) {
	  ...
	  var choice = choices[0];
	  corp.AI.preferred = { title: "长生精华液", option: choice }; //title must match currentPhase.title for AI to fire
	}
  },
  ...
  responseOnApproachServer: {
    ...
    Resolve: function (params) {
      ...
      if (corp.AI != null) {
        ...
        corp.AI.preferred = { title: "无明虚空", option: choice }; //title must match currentPhase.title for AI to fire
      }
    },
    text: "Whenever the Runner approaches this server",
  },
  ...
  responseOnEncounter: {
    ...
    Resolve: function (params) {
        ...
        if (runner.AI != null) {
          ...
          runner.AI.preferred = { title: "欢乐屋", option: choice }; //title must match currentPhase.title for AI to fire
        }
    },
  },
...
cardSet[30057] = {
  ...
  AIWouldPlayBeforeScore: function (cardToScore, serverToScoreIn) {
	if (!CheckTags(1) && cardToScore.title == "轨道压制") return true; //worth it to do that 4 meat damage!
    return false;
  },
};
cardSet[30058] = {
...
cardSet[30060] = {
  ...
  responseOnScored: {
    ...
    Resolve: function (params) {
      ...
      choices.push({ card: null, label: "继续", button: "继续" });
      var decisionCallback = function (params) {
  ...
  responseOnCorpTurnBegins: {
    ...
    Resolve: function (params) {
      ...
      if (corp.AI != null) {
        ...
        corp.AI.preferred = { title: "清算所", option: choice }; //title must match currentPhase.title for AI to fire
      }
    },
  ...
  subroutines: [
    {
      Resolve: function () {
        ...
        if (corp.AI != null && choicesA.length > 1) {
          ...
          corp.AI.preferred = { title: "弩炮", option: choice }; //title must match currentPhase.title for AI to fire
        }
      },
  ...
  responseOnScored: {
    Enumerate: function () {
      if (scoringFromServer == GetServer(this)) {
        ...
        choices.push({ card: null, label: "继续", button: "继续" }); //even if there are no non-agenda cards, you can legally search and fail  (more info here: http://ancur.wikia.com/wiki/Democracy_and_Dogma_UFAQ#Mumbad_City_Hall)
        return choices;
      }
      return [];
    },
...
cardSet[30069] = {
  ...
  SharedResolve: function (params) {
    ...
    //**AI code
      ...
      corp.AI.preferred = { title: "以儆效尤", option: choice }; //title must match currentPhase.title for AI to fire
    }
  },
  responseOnScored: {
...
cardSet[30076] = {
  ...
  Tutorial: function(str) {
    if ((str=="Corp Mulligan")||(str=="Runner Mulligan")) {
		if (globalProperties.agendaPointsToWin == 6) TutorialMessage("这局游戏中，我们将使用《核心网关》起始牌组。\n\n第一位获得6点议案分数的玩家获胜（正常游戏需要7分）。\n\n要撤销一个决定，使用游戏菜单（帽子图标）中的倒回功能。");
		else TutorialMessage("公司现在有以下能力：\n• @, 2$: 销毁一个资源（仅当潜袭者被锁定时才能使用）\n• @@@: 清除病毒指示物\n\n潜袭者现在有以下能力：\n• @, 2$: 移除1枚锁定标记");
	}
