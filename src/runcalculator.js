  ...
  CalculatePieceBegin(data) {
    ...
    if (!data.incomplete) {
	  ...
	  if (breach) {
		  ...
		  if (mightAccessAnAgenda && corp.identityCard.title=="人间会社：自我进化") {
		  ...
		  if (numUnknownCardsInRoot > 0) mightHitHokusai = true;
			...
			for (var i = 0; i < data.server.root.length; i++) {
			  if (data.server.root[i].title == "北斋网阵") {
		  ...
		  for (var i = 0; i < knownCardsInRoot.length; i++) {
			...
			if (knownCardsInRoot[i].title == "荨麻暗码") {
				var approachEffect = [];
      ...
      if (
        runnerAI._copyOfCardExistsIn("逐月狼秘密开发部", knownCardsInRoot)
      ) {
        //create a fork
